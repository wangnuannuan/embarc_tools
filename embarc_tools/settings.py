...
LIBRARIES = ["clib", "console", "unittest"]
...
BUILD_INFO_NAMES = ['EMBARC_ROOT', 'OUT_DIR_ROOT', 'BUILD_OPTION', 'APPLICATION_NAME', 'APPLICATION_LINKSCRIPT', 'APPLICATION_ELF', 'APPLICATION_BIN', 'APPLICATION_HEX', 'APPLICATION_MAP', 'APPLICATION_DUMP', 'APPLICATION_DASM', 'PERIPHERAL']
...
class embARC(object):
    def __init__(self, embarc_file="embarc.json"):
    ...
    def get_path(self, path):  # find if path exists in embarc.json
        fl = os.path.join(self.path, self.file)
        if not os.path.exists(fl):
            self.cfg_dict = dict()
            generate_json(self.cfg_dict, fl)
    ...
    def set_path(self, name, source_type, path, url=None):
        ...
        if not os.path.exists(fl):
            ...
            generate_json(self.cfg_dict, fl)
        path = path.replace("\\", "/")
        if not self.is_embarc_root(path):
    ...
    def list_path(self, show=False, current=None):
        ...
        try:
            ...
            if self.cfg_dict:
                ...
                print_string("There is no valid embarc root path in record")

        except IOError:
            raise IOError("Can not open file %s ." % fl)

    def is_embarc_root(self, path):
        if os.path.exists(path) and os.path.isdir(path):
            for files in EMBARC_SUB_DIRS:
    ...
    def check_embarc(self, path):
        update = False
        if path:
            embarc_root = path.replace("\\", "/")
        else:
            embarc_root = path
        if not embarc_root or not self.is_embarc_root(embarc_root):
            update = True
            msg = "Current EMBARC_ROOT is not a valid embarc root"
            level = "warning"
            print_string(msg, level)
            self.list_path(show=True, current=None)
            while True:
                input_root = get_input("Choose embarc root or set another path as embarc root: ")
                if not self.is_embarc_root(input_root):
                ...
                if not self.is_embarc_root(input_root):
                    ...
                    level = "warning"
                    print_string(msg, level)
                    print_string("Please set a valid embarc root or download embarc embarc first")
                    continue

                break
            embarc_root = input_root.replace("\\", "/")
        return embarc_root, update
        ...
        with open(makefile) as f:
            ...
            for line in lines:
                ...
                if line.startswith("EMBARC_ROOT"):
                    ...
                    embarc_root = os.path.normpath(os.path.join(os.getcwd(), relative_root))
